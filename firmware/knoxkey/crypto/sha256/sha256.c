 ...
 * Modified by Martin Endler for the knoxkey project.
 */
...
static void sha256_transform(sha256_ctx_t *const ctx, const uint8_t *const data) {
...
void sha256_init(sha256_ctx_t *const ctx) {
	ctx->data_length = 0;
	ctx->bit_length = 0;
...
void sha256_update(sha256_ctx_t *const ctx, const uint8_t *const data, const size_t data_length) {
	for (size_t i = 0; i < data_length; ++i) {
		ctx->data[ctx->data_length] = data[i];
		ctx->data_length++;
		if (ctx->data_length == knoxkey_SHA256_BLOCK_SIZE) {
...
void sha256_final(sha256_ctx_t *const ctx, uint8_t *const hash) {
	...
	ctx->data[63] = ctx->bit_length;
	ctx->data[62] = ctx->bit_length >> 8;
	ctx->data[61] = ctx->bit_length >> 16;
	ctx->data[60] = ctx->bit_length >> 24;
	ctx->data[59] = ctx->bit_length >> 32;
	ctx->data[58] = ctx->bit_length >> 40;
	ctx->data[57] = ctx->bit_length >> 48;
	ctx->data[56] = ctx->bit_length >> 56;
	sha256_transform(ctx, ctx->data);
...
const hash_alg_t hash_alg_sha256 = {
	...
	.output_size = knoxkey_SHA256_OUTPUT_SIZE,
	.block_size = knoxkey_SHA256_BLOCK_SIZE,

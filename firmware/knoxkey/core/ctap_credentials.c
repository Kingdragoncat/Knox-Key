 ...
 *
 * Currently, knoxkey does not support a built-in user verification method,
...
static uint8_t ensure_uv_option_false(const bool pinUvAuthParam_present, CTAP_mc_ga_options *options) {
	...
	if (uv) {
		...
		//       (as is the case with the current version of knoxkey),
...
uint8_t ctap_make_credential(ctap_state_t *const state, CborValue *const it, CborEncoder *const encoder) {
	...
	// 6. (not applicable to knoxkey) If the alwaysUv option ID is present and true then ...
	...
	// 8. (not applicable to knoxkey) Else: (the makeCredUvNotRqd option ID in authenticatorGetInfo's response
	...
	// 13. (not applicable to knoxkey) If evidence of user interaction was provided as part of Step 11
...
uint8_t ctap_get_assertion(ctap_state_t *const state, CborValue *const it, CborEncoder *const encoder) {
	...
	// 5. (not applicable to knoxkey) If the alwaysUv option ID is present and true and ...
	...
	// 8. (not applicable to knoxkey) If evidence of user interaction was provided as part of Step 6.2
	//    (i.e., by invoking performBuiltInUv()): ...

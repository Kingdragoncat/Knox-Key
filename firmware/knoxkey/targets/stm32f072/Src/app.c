/**
 * @file    app.c
 * @brief   Knox Key application logic — STM32F072 target
 *
 * Adapted from LionKey's stm32h533 app.c.
 * Uses software crypto (micro-ecc, tiny-AES-c, sha256) since the STM32F072
...
/* --- Storage (RAM-based for now) ---
 * NOTE: With only 16KB RAM and ~15KB used by CTAPHID protocol buffers,
 * we keep storage minimal. For production, flash-based storage is needed.
 */
static uint8_t memory[2 * 1024];
